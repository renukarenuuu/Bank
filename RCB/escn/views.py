    ...
    if request.method=="POST":
        ...
        if data:
            if data.acc_number==acc:
                ...
                messages.error(request,'Please enter your Account Number','pin.html')
            ...
            if int(data.get_pin())==int(pin):
               if amt>=100:
                   if amt<=10000:
                       ...
                       data.save()
                       send_mail("AMOUNT CREDITED",f"amount{amt} has been credited to ur account{data.acc_number}via ATM ur currnet available balance is{data.balance}",settings.EMAIL_HOST_USER,[data.email],fail_silently=True)
                       return redirect("home")
                   else:
            ...
            if int(data.get_pin())==int(pin):
               if amt>=100:
                   ...
                   if amt<=bal:
                       data.balance=bal-amt
                       data.save()
                       send_mail("AMOUNT DEBITED",f"amount{amt} has been debited from ur account{data.acc_number}via ATM ur currnet available balance is{data.balance}",settings.EMAIL_HOST_USER,[data.email],fail_silently=True)
                       return redirect("home")
                   else:
